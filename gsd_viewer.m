...
if isempty(hgsdviewMain)
    ...
    % --- Detector parameters
    % ------------------------------------------------------------------%%%
    config.visopt.Inst(1).par     = repmat([0.053 0 5],192,1);  % [a b TOA]
    %%% ----------------------------------------------------------------%%%
    initFigure(config);
else
    figure(hgsdviewMain);
...
heditExpInfo(1) = uicontrol('Style','edit',...
    ...
    'String',num2str(config.visopt.Inst(1).par(1,3),'%.10g'),...
    'HorizontalAlignment','center',...
...
heditExpInfo(2) = uicontrol('Style','edit',...
    ...
    'backgroundcolor',[1 1 1],...
    'fontsize',config.fs.edbox,...
    'String',num2str(config.visopt.Inst(1).par(1,2),'%.10g'),...
    'HorizontalAlignment','center',...
    'CallBack',@expinfo_det_Callback,...
  ...
  function expinfo_det_Callback(hObject,~)
      ...
      switch tag
          ...
          case 'expinfo_pop_pxno'
              ...
              fprintf('select det-%d, px-%d, [%.8f %.8f %.8f]\n',detno, pxno, detpar(1), detpar(2), detpar(3))
              h.expinfo_edit_ch2ea.String = num2str(detpar(1),'%.8g');
              h.expinfo_edit_ch2eb.String = num2str(detpar(2),'%.8g');
              h.expinfo_edit_toa.String = num2str(detpar(3),'%.8g');
          ...
          case 'expinfo_edit_ch2eb'
              detno = h.expinfo_pop_detno.Value;
              pxno  = h.expinfo_pop_pxno.Value;
              orgpar = h.config.visopt.Inst(detno).par(pxno,2);
              par = str2num(get(hObject,'String'));
              if isempty(par) || par == 0
                  set(hObject,'String',num2str(orgpar,'%.10g'));
              elseif par < 0
                  set(hObject,'String',num2str(abs(par),'%.10g'));
                  h.config.visopt.Inst(detno).par(pxno,2) = abs(par);
                  h.expinfo_table_detpar.Data{pxno,2} = abs(par);
              else
                  h.config.visopt.Inst(detno).par(pxno,2) = par;
                  h.expinfo_table_detpar.Data{pxno,2} = par;
              end
          case 'expinfo_edit_toa'
              detno = h.expinfo_pop_detno.Value;
              pxno  = h.expinfo_pop_pxno.Value;
              orgpar = h.config.visopt.Inst(detno).par(pxno,3);
              par = str2num(get(hObject,'String'));
              if isempty(par) || par == 0
                  set(hObject,'String',num2str(orgpar,'%.10g'));
              elseif par < 0
                  set(hObject,'String',num2str(abs(par),'%.10g'));
                  h.config.visopt.Inst(detno).par(pxno,3) = abs(par);
                  h.expinfo_table_detpar.Data{pxno,3} = abs(par);
              else
                  h.config.visopt.Inst(detno).par(pxno,3) = par;
                  h.expinfo_table_detpar.Data{pxno,3} = par;
              end
          otherwise
              fprintf('No yet set!!\n');
      end
          ...
          case 'visopt_pop_xunit'
              ...
              switch xunit
                  case 1
                      ...
                      switch org_unit
                          case 'E'
                              new_max = min([round((org_limit(2)-detinfo.par(1,2))/detinfo.par(1,1)) 4096]);
                              new_min = max([round((org_limit(1)-detinfo.par(1,2))/detinfo.par(1,1)) 0]);
                          ...
                          case 'd'
                              Emin = 12.398419057638671/(2*org_limit(2)*sind(detinfo.par(1,3)/2));
                              Emax = min([12.398419057638671/(2*org_limit(1)*sind(detinfo.par(1,3)/2)) 250]);
                              new_max = min([round((Emax-detinfo.par(1,2))/detinfo.par(1,1)) 4096]);
                              new_min = max([round((Emin-detinfo.par(1,2))/detinfo.par(1,1)) 0]);
                              h.visopt_edit_xrange1.String = num2str(new_min,'%.10g');
                              h.visopt_edit_xrange2.String = num2str(new_max,'%.10g');
                              h.config.visopt.datalim{1} = [new_min new_max];
                      end
                  ...
                  case 2
                      ...
                      switch org_unit
                          case 'ch'
                              new_max = round((org_limit(2)*detinfo.par(1,1)+detinfo.par(1,2)));
                              new_min = max([round((org_limit(1)*detinfo.par(1,1)+detinfo.par(1,2))) 0 ]);
                              h.visopt_edit_xrange1.String = num2str(new_min,'%.10g');
                              h.visopt_edit_xrange2.String = num2str(new_max,'%.10g');
                              h.config.visopt.datalim{1} = [new_min new_max];
                          case 'd'
                              new_min = round(12.398419057638671/(2*org_limit(2)*sind(detinfo.par(1,3)/2)));
                              new_max = min([round(12.398419057638671/(2*org_limit(1)*sind(detinfo.par(1,3)/2))) 250]);
                              h.visopt_edit_xrange1.String = num2str(new_min,'%.10g');
                              h.visopt_edit_xrange2.String = num2str(new_max,'%.10g');
                              h.config.visopt.datalim{1} = [new_min new_max];
                      end                      
                  ...
                  case 3
                      ...
                      switch org_unit
                          case 'ch'
                              E_max = round((org_limit(2)*detinfo.par(1,1)+detinfo.par(1,2)));
                              E_min = max([round((org_limit(1)*detinfo.par(1,1)+detinfo.par(1,2))) 0.1]);
                              new_max = min([12.398419057638671/E_min/2/sind(detinfo.par(1,3)/2) 5]);
                              new_min = max([12.398419057638671/E_max/2/sind(detinfo.par(1,3)/2) 0]);
                              h.visopt_edit_xrange1.String = num2str(new_min,'%.10g');
                              h.visopt_edit_xrange2.String = num2str(new_max,'%.10g');
                              h.config.visopt.datalim{1} = [new_min new_max];
                          case 'E'
                              new_max = min([12.398419057638671/org_limit(1)/2/sind(detinfo.par(1,3)/2) 5]);
                              new_min = max([12.398419057638671/org_limit(2)/2/sind(detinfo.par(1,3)/2) 0]);                              
                              h.visopt_edit_xrange1.String = num2str(new_min,'%.10g');
                              h.visopt_edit_xrange2.String = num2str(new_max,'%.10g');
                              h.config.visopt.datalim{1} = [new_min new_max];
                      end                        
...
function main_load_Callback(~,~)
  ...
  if fname ~=0
      ...
      detno = h.expinfo_pop_detno.Value;
      pxno  = h.expinfo_pop_pxno.Value;
      h.expinfo_edit_ch2ea.String = num2str(Inst(detno).par(pxno,1),'%.10g');
      h.expinfo_edit_ch2eb.String = num2str(Inst(detno).par(pxno,2),'%.10g');
      h.expinfo_edit_toa.String = num2str(Inst(detno).par(pxno,3),'%.10g');
      % update table
      %num2cell(Inst(detno).par)
